...
"""
Script 15: Run LPI on the v3 panel (7 features, single seed).

Usage:
    python3.11 scripts/15_run_lpi_v3.py
...
def _generate_report_v3(result, cfg, output_dir, universe_size):
    """Generate metrics, figures and print verdict for the v3 run."""
    import matplotlib
    matplotlib.use("Agg")
    ...
    ax.plot(fpr, tpr, lw=2, color="darkorange", label=f"AUC = {auc_mean:.4f} (v3, 7 feat)")
    ax.plot([0, 1], [0, 1], "k--", lw=1)
    ax.set_xlabel("False Positive Rate")
    ax.set_ylabel("True Positive Rate")
    ax.set_title("LPI v3 (7 features) — Curva ROC (OOS)")
    ax.legend()
    plt.tight_layout()
    plt.savefig(fig_dir / "roc_curve_v3.png", dpi=120)
    plt.close()
    ...
    ax.set_title("Score Distribution — v3 (7 features)")
    ax.legend()
    plt.tight_layout()
    plt.savefig(fig_dir / "score_distribution_v3.png", dpi=120)
    plt.close()

    # Calibration
    fraction_pos, mean_pred = calibration_curve(y, scores, n_bins=10, strategy="quantile")
    fig, ax = plt.subplots(figsize=(5, 5))
    ax.plot(mean_pred, fraction_pos, "s-", color="darkorange", label="LPI v3")
    ax.plot([0, 1], [0, 1], "k--", lw=1)
    ax.set_title("Calibration — v3 (7 features)")
    ax.legend()
    plt.tight_layout()
    plt.savefig(fig_dir / "calibration_v3.png", dpi=120)
    plt.close()
    ...
    pd.DataFrame([main_results]).to_csv(tab_dir / "main_results_v3.csv", index=False)
    quintile_df.to_csv(tab_dir / "quintile_stats_v3.csv", index=False)

    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    lines = [
        "",
        "====== LPI v3 (7 features: CBOE 4 + OHLCV 3) ======",
        f"Features: {FEATURE_NAMES_V3}",
...
def main():
    ...
    logger.info("Running LPI v3 (7 features, seed=42) …")
    ...
    np.save(root / "data" / "processed" / "scores_oos_v3.npy", result["scores_oos"])
    np.save(root / "data" / "processed" / "y_oos_v3.npy", result["y_oos"])
    np.save(root / "data" / "processed" / "k_star_v3.npy", np.array([result["k_star"]]))
